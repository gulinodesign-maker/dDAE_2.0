 ...
 */
const BUILD_VERSION = "dDAE_2.170";
...
// dDAE_2.170 — error overlay: evita blocchi silenziosi su iPhone PWA
...
/* dDAE_2.170 — Tap counters: Adulti / Bambini <10 (tap increment, long press 0.5s = reset) */
...
// dDAE_2.170 — Fix contrast icone topbar: se un tasto appare bianco su iOS, l'icona bianca diventa invisibile.
...
if (page === "orepulizia") { initOrePuliziaPage().catch(e=>toast(e.message)); }
  ...
  // dDAE_2.170: fallback visualizzazione Pulizie
  try{
    if (page === "pulizie"){
...
// ===== dDAE_2.170 — Multi prenotazioni per stesso nome =====
  ...
  if (hdActions && !hdActions.__bound){
    ...
    hdActions.addEventListener("click", async (e) => {
      ...
      if (btn.hasAttribute("data-guest-del")){
        ...
        // ✅ dDAE_2.170: dopo cancellazione, vai SUBITO alla guest list (UX immediata su iOS)
...
// periodo iniziale
  ...
  // dDAE_2.170: salvataggio PULIZIE per-stanza (evita generazione righe/report inutili)
...
function __fitCalendarioMonthLandscape(){
  try{
    ...
    // dDAE_2.170: in vista mese su iPad landscape usa tutta la larghezza disponibile (margine 10px L/R)
...
// --- FIX dDAE_2.170: renderSpese allineato al backend ---
...
// --- FIX dDAE_2.170: delete reale ospiti ---
...
// --- FIX dDAE_2.170: mostra nome ospite ---
(function(){
  const orig = window.renderOspiti;
  if (!orig) return;
  window.renderOspiti = function(){
