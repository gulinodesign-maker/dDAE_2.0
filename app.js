...
function renderRoomsReadOnly(ospite){
  ...
  // Persone + date (primo gruppo): in sola lettura usiamo lo stesso layout dei gruppi successivi
  const a0 = Number(main?.adulti ?? main?.adults ?? 0) || 0;
  const k0 = Number(main?.bambini_u10 ?? main?.kidsU10 ?? 0) || 0;
  const ci0 = formatLongDateIT(main?.check_in || main?.checkIn || "") || "—";
  const co0 = formatLongDateIT(main?.check_out || main?.checkOut || "") || "—";
  ...
  // Matrimonio: pallino verde con "M" (solo in sola lettura)
  const marriageOn = !!(main?.matrimonio);

  const rightHTML = (pillHTML || marriageOn)
    ? `<div class="stay-right">${marriageOn ? `<span class="marriage-dot" aria-label="Matrimonio">M</span>` : ``}${pillHTML}</div>`
    : ``;

  const mainHTML = `
    <div class="rooms-readonly-main" aria-label="Gruppo prenotazione 1">
      <div class="ro-grid" aria-label="Persone">
        <div class="ro-box"><div class="ro-lbl">Adulti</div><div class="ro-val">${a0}</div></div>
        <div class="ro-box"><div class="ro-lbl">Bambini &lt; 10</div><div class="ro-val">${k0}</div></div>
      </div>
      <div class="ro-grid" aria-label="Date">
        <div class="ro-box"><div class="ro-lbl">Check-in</div><div class="ro-val">${escapeHtml(ci0)}</div></div>
        <div class="ro-box"><div class="ro-lbl">Check-out</div><div class="ro-val">${escapeHtml(co0)}</div></div>
      </div>
      <div class="rooms-readonly-wrap">
        ${stackHTML}
        ${rightHTML}
      </div>
    </div>
  ...
  ro.innerHTML = mainHTML + extraHTML;
}
...
function setGuestFormViewOnly(isView, ospite){
  ...
  const gg = document.getElementById("guestGroups");
  if (gg) gg.hidden = !!isView;
