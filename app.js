 ...
 */
const BUILD_VERSION = "dDAE_2.138";
...
// dDAE_2.138 — error overlay: evita blocchi silenziosi su iPhone PWA
...
/* dDAE_2.138 — Tap counters: Adulti / Bambini <10 (tap increment, long press 0.5s = reset) */
...
if (page === "orepulizia") { initOrePuliziaPage().catch(e=>toast(e.message)); }
  ...
  // dDAE_2.138: fallback visualizzazione Pulizie
  try{
    if (page === "pulizie"){
...
// ===== dDAE_2.138 — Multi prenotazioni per stesso nome =====
  ...
  if (hdActions && !hdActions.__bound){
    ...
    hdActions.addEventListener("click", async (e) => {
      ...
      if (btn.hasAttribute("data-guest-del")){
        ...
        // ✅ dDAE_2.138: dopo cancellazione, vai SUBITO alla guest list (UX immediata su iOS)
...
// --- FIX dDAE_2.138: renderSpese allineato al backend ---
...
// --- FIX dDAE_2.138: delete reale ospiti ---
...
// --- FIX dDAE_2.138: mostra nome ospite ---
(function(){
  const orig = window.renderOspiti;
  if (!orig) return;
  window.renderOspiti = function(){
