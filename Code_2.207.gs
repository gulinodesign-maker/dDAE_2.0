...
function handleRequest_(e, method) {
  try {
    ...
    switch (action) {
      ...
      case "servizi":
        return handleServizi_(e, method);
      case "spese":
      ...
      case "piscina":
        return handlePiscina_(e, method);
      case "impostazioni":
...
function handlePiscina_(e, method){
  method = String(method||"GET").toUpperCase();
  const sh = getSheet_(SHEETS.PISCINA);
  const ctx = getCtx_(e);
  if (!ctx.user_id) return jsonError_("user_id mancante");
  if (!ctx.anno) return jsonError_("anno mancante");

  const required = [
    "id","user_id","anno","timestamp_report","origine",
    "cloro_attivo_libero","cloro_attivo_combinato","ph","temp_acqua",
    "createdAt","updatedAt"
  ];
  ensureHeaders_(sh, required);

  if (method === "GET"){
    const { rows } = readAll_(sh);
    let out = filterByUserAnno_(rows, ctx);

    // Filtri opzionali (per performance): from/to in formato YYYY-MM-DD sul giorno del timestamp_report
    ...
    if (from || to){
      out = out.filter(r => {
        const ts = String(pick_(r.timestamp_report, r.timestamp, r.ts, "")).trim();
        if (!ts) return false;
  ...
  if (method === "POST" || method === "PUT"){
    ...
    const items = Array.isArray(payload && payload.rows) ? payload.rows
                : (Array.isArray(payload) ? payload : [payload]);
    if (!items || !items.length) return jsonError_("Nessun report piscina");
    ...
    for (const item of items){
      ...
      obj.id = String(pick_(item.id, item.ID, item.Id, "")).trim() || uuid_();
      obj.user_id = ctx.user_id;
      obj.anno = ctx.anno;

      obj.timestamp_report = String(pick_(item.timestamp_report, item.timestamp, item.ts, "")).trim();
      if (!obj.timestamp_report) return jsonError_("timestamp_report mancante");

      obj.origine = String(pick_(item.origine, item.source, "")).trim();
      obj.cloro_attivo_libero = toNumOrEmpty_(pick_(item.cloro_attivo_libero, item.cloro_libero, item.cloro, ""));
      obj.cloro_attivo_combinato = toNumOrEmpty_(pick_(item.cloro_attivo_combinato, item.cloro_combinato, ""));
      obj.ph = toNumOrEmpty_(pick_(item.ph, item.pH, ""));
      obj.temp_acqua = toNumOrEmpty_(pick_(item.temp_acqua, item.temperatura, item.temp, ""));

      obj.createdAt = String(pick_(item.createdAt, item.created_at, nowIso));
  ...
  return jsonError_("Metodo non supportato (piscina)");
}
...
function deletePulizieByIdOrKey_(sh, key){
  key = key || {};
  const id = String(key.id || "").trim();
  const userId = String(key.user_id || "").trim();
  const anno = String(key.anno || "").trim();
  const dataKey = isoDatePulizie_(String(key.data || "").trim());
  const stanzaKey = String(key.stanza || "").trim();
  ...
  const idxData = (headers.indexOf("data") >= 0) ? headers.indexOf("data") : headers.indexOf("Data");
  const idxStanza = (headers.indexOf("stanza") >= 0) ? headers.indexOf("stanza") : headers.indexOf("Stanza");

  const canMatchKey = !!(userId && anno && dataKey && stanzaKey && idxUser >= 0 && idxAnno >= 0 && idxData >= 0 && idxStanza >= 0);
  ...
  for (let r = 1; r < data.length; r++){
    ...
    if (idxId >= 0 && id){
      if (String(data[r][idxId]).trim() === id) match = true;
    }

    if (!match && canMatchKey){
      const ru = String(data[r][idxUser]).trim();
      const ra = String(data[r][idxAnno]).trim();
      const rd = isoDatePulizie_(String(data[r][idxData] || "").trim());
      const rs = String(data[r][idxStanza]).trim();

      if (ru === userId && ra === anno && rd === dataKey && rs === stanzaKey) match = true;
    }
...
function handleOspiti_(e, method) {
  ...
  if (method === "DELETE") {
    ...
    const ctx = getCtx_(e);
    const accountId = String(ctx.user_id || "").trim();
    const anno = String(ctx.anno || "").trim();

    // motivo (opzionale) - default: cancellazione
    const deleteReason = String(e.parameter.delete_reason || e.parameter.deleteReason || "cancellazione").trim() || "cancellazione";

    // recupera snapshot dell'ospite prima della cancellazione
    let guestRow = null;
    try{
      const all = readAll_(sh);
      const rows = all && all.rows ? all.rows : [];
      guestRow = rows.find(r => String(r.id || r.ID || "").trim() === id) || null;
    }catch(_){ guestRow = null; }

    // archivia su ospiti_eliminati (audit + report)
    try{
      const shDel = getSheet_(SHEETS.OSPITI_ELIMINATI);
      const nowIso = new Date().toISOString();
      const idArch = "del_" + Date.now() + "_" + Math.floor(Math.random()*100000);

      const createdOrig = String(pick_(guestRow && (guestRow.createdAt || guestRow.created_at || guestRow.createdAt_originale || guestRow.createdAtOriginale), "") || "").trim();
      ...
      const payload = {
        ...
        id_archivio: idArch,
        deletedBy: accountId,
        createdAt_originale: createdOrig,
        anno: anno,
        deletedAt: nowIso,
        delete_reason: deleteReason,
        nome: String(pick_(guestRow && (guestRow.nome || guestRow.name), "") || ""),
        check_in: String(pick_(guestRow && (guestRow.check_in || guestRow.checkIn || guestRow.checkin), "") || ""),
        check_out: String(pick_(guestRow && (guestRow.check_out || guestRow.checkOut || guestRow.checkout), "") || ""),
        stanze: String(pick_(guestRow && guestRow.stanze, "") || ""),
        importo_booking: String(pick_(guestRow && (guestRow.importo_booking || guestRow.importoBooking), "") || ""),
        importo_prenota: String(pick_(guestRow && (guestRow.importo_prenotazione || guestRow.importo_prenota || guestRow.importoPrenota || guestRow.importoPrenotazione), "") || ""),
        acconto_importo: String(pick_(guestRow && (guestRow.acconto_importo || guestRow.accontoImporto), "") || ""),
      };

      upsertById_(shDel, payload, "id_archivio");
    }catch(_){ }
...
function handleOspitiEliminati_(e, method){
  ...
  if (method === "GET") {
    ...
    rows = filterByUserAnno_(rows, getCtx_(e));

    const from = String((e.parameter && e.parameter.from) ? e.parameter.from : "").trim();
    const to = String((e.parameter && e.parameter.to) ? e.parameter.to : "").trim();
    if (from || to){
      rows = (Array.isArray(rows) ? rows : []).filter(r => {
        const d = String(pick_(r.deletedAt, r.deleted_at, r.deletedat, r.timestamp) || "").trim();
        if (!d) return false;
  ...
  return jsonError_("Metodo non supportato per ospiti_eliminati: " + method);
}
...
function normalizeOspite_(d) {
  ...
  const telefono = String(pick_(d.telefono, d.tel, d.phone, d.mobile, d.cell, d.cellulare));
  const email = String(pick_(d.email, d.mail, d.e_mail));
  ...
  const matrimonio = String(pick_(d.matrimonio, d.wedding, d.m, d.M));
  const gruppoG = String(pick_(d.gruppo_g, d.gruppoG, d.flag_g, d.flagG, d.g, d.G, d.gruppo, d.group_g, d.groupG));
  const colC = String(pick_(d.col_c, d.colC, d.flag_c, d.flagC, d.c, d.C, d.colc));
...
// =========================
// SERVIZI (per-ospite, multi-riga)
// =========================

function handleServizi_(e, method){
  const sh = getSheet_(SHEETS.SERVIZI);
  const ctx = getCtx_(e);

  if (method === "GET"){
    const { rows } = readAll_(sh);
    let scoped = filterByUserAnno_(rows, ctx);

    const ospiteId = String(e.parameter.ospite_id || e.parameter.ospiteId || "").trim();
    if (ospiteId){
      scoped = scoped.filter(r => String(r.ospite_id || r.ospiteId || "").trim() === ospiteId);
    }
    return jsonOk_(scoped);
  }

  if (method === "POST" || method === "PUT"){
    const payload = parseBody_(e) || {};
    const ospiteId = String(pick_(payload.ospite_id, payload.ospiteId, payload.guest_id, payload.guestId)).trim();
    if (!ospiteId) return jsonError_("ospite_id mancante in payload servizi");

    const arr = Array.isArray(payload.servizi) ? payload.servizi
      : Array.isArray(payload.items) ? payload.items
      : [];

    // elimina servizi esistenti per questo ospite NELLO STESSO tenant (user_id + anno)
    const deleted = deleteServiziScoped_(sh, ospiteId, ctx);
    ...
    for (const it of arr){
      const obj = normalizeServizio_(it, ospiteId, ctx, nowIso);
      sh.appendRow(buildRowFromHeaders_(headers, obj));
      inserted++;
    }

    return jsonOk_({ ospite_id: ospiteId, deleted: deleted, inserted: inserted });
  }

  return jsonError_("Metodo non supportato per servizi: " + method);
}

function deleteServiziScoped_(sh, ospiteId, ctx){
  const data = sh.getDataRange().getValues();
  if (!data || data.length < 2) return 0;
  const headers = (data[0] || []).map(h => String(h).trim());

  const idxO = headers.indexOf("ospite_id");
  const idxU = headers.indexOf("user_id");
  const idxA = headers.indexOf("anno");
  ...
  const toDelete = [];
  for (let r=1; r<data.length; r++){
    const o = String(data[r][idxO]).trim();
    if (o !== String(ospiteId).trim()) continue;

    if (uid && idxU >= 0){
      const ru = String(data[r][idxU]).trim();
      if (ru !== uid) continue;
    }
    if (yr && idxA >= 0){
      const ra = String(data[r][idxA]).trim();
      if (ra !== yr) continue;
    }
    toDelete.push(r+1);
  }
...
function normalizeServizio_(item, ospiteId, ctx, nowIso){
  item = item || {};
  const obj = {};

  obj.id = String(pick_(item.id, item.ID, item.Id, "")).trim() || uuid_();
  obj.user_id = String(pick_(item.user_id, item.userId, ctx && ctx.user_id, "")).trim();
  obj.anno = String(pick_(item.anno, item.year, ctx && ctx.anno, "")).trim();
  obj.ospite_id = String(pick_(item.ospite_id, item.ospiteId, ospiteId, "")).trim();

  obj.servizio = String(pick_(item.servizio, item.nome, item.name, "")).trim();
  obj.descrizione = String(pick_(item.descrizione, item.desc, item.note, "")).trim();

  const imp = parseFloat(String(pick_(item.importo, item.amount, 0)).replace(",", ".")) || 0;
  obj.importo = imp;

  const qty = parseFloat(String(pick_(item.qty, item.qta, 1)).replace(",", ".")) || 1;
  obj.qty = qty;

  obj.isDeleted = String(pick_(item.isDeleted, item.is_deleted, item.deleted, ""));

  // timestamps
  obj.createdAt = String(pick_(item.createdAt, item.created_at, nowIso || new Date().toISOString()));
  obj.updatedAt = String(pick_(item.updatedAt, item.updated_at, nowIso || new Date().toISOString()));
  ...
  if (method === "POST" || method === "PUT") {
    ...
    for (const item of items) {
      ...
      if (isPulizieAllZero_(obj)){
        ...
        // Importante: in passato alcuni record potrebbero avere id diversi/vecchi → cancella anche per chiave logica (user_id+anno+data+stanza).
        try{ deleted += deletePulizieByIdOrKey_(sh, { id: obj.id, user_id: obj.user_id, anno: obj.anno, data: obj.data, stanza: obj.stanza }); }catch(_){ }
        continue;
      }
    ...
    return jsonOk_({ saved: saved, deleted: deleted });
  }
...
function isPulizieAllZero_(obj){
  obj = obj || {};
  const fields = ["MAT","SIN","FED","TDO","TFA","TBI","TAP","TPI"];
