...
function handleOspiti_(e, method) {
  ...
  if (method === "DELETE") {
    ...
    // 1) Recupera dati ospite (scoped) prima di eliminare
    const { rows } = readAll_(sh);
    const scoped = filterByUserAnno_(rows, ctx);
    const guest = (scoped || []).find(r => String(r.id || "").trim() === id);

    // 2) Archivia in ospiti_eliminati (audit cancellazioni)
    if (guest) {
      ...
      ensureHeaders_(shDel, required);

      const nowIso = new Date().toISOString();
      const archiveId = "del_" + Date.now() + "_" + Math.floor(Math.random()*100000);

      const archived = {
        id_originale: id,
        account_id: String(pick_(guest.account_id, guest.accountId, guest.user_id, guest.userId, ctx.user_id, "")).trim(),
        id_archivio: archiveId,
        deletedBy: String(pick_(e.parameter._actor_id, e.parameter.actor_id, e.parameter.actorId, "")).trim(),
        createdAt_originale: String(pick_(guest.createdAt_originale, guest.createdAt, guest.created_at, "")).trim(),
        anno: String(pick_(guest.anno, guest.year, ctx.anno, "")).trim(),
        deletedAt: nowIso,
        delete_reason: String(pick_(e.parameter.delete_reason, e.parameter.deleteReason, e.parameter.reason, "cancellazione")).trim(),
        nome: String(pick_(guest.nome, "")).trim(),
        check_in: String(pick_(guest.check_in, guest.checkIn, "")).trim(),
        check_out: String(pick_(guest.check_out, guest.checkOut, "")).trim(),
        stanze: String(pick_(guest.stanze, "")).trim(),
        importo_booking: String(pick_(guest.importo_booking, guest.importoBooking, "")).trim(),
        importo_prenota: String(pick_(guest.importo_prenota, guest.importo_prenotazione, guest.importoPrenota, guest.importoPrenotazione, "")).trim(),
        acconto_importo: String(pick_(guest.acconto_importo, guest.accontoImporto, "")).trim(),
      };

      // garantisce account_id/anno
      if (!archived.account_id) archived.account_id = String(ctx.user_id || "").trim();
      if (!archived.anno) archived.anno = String(ctx.anno || "").trim();

      upsertById_(shDel, archived, "id_archivio");
    }

    // 3) Elimina record ospite + stanze collegate
